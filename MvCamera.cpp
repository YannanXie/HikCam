...
// ch:设置显示窗口句柄 | en:Set Display Window Handle
//int CMvCamera::DisplayOneFrame1(MV_DISPLAY_FRAME_INFO* pDisplayInfo)
//{
//    return MV_CC_DisplayOneFrame(m_hDevHandle1, pDisplayInfo);
//}
...
{
    return MV_CC_GetIntValueEx(m_hDevHandle, strKey, pIntValue);    // 获取 Integer 属性值
    // m_hDevHandle：设备句柄； strKey：属性键值； pIntValue：返回给调用者有关设备属性结构体指针
}

int CMvCamera::GetIntValue2(IN const char* strKey, OUT MVCC_INTVALUE *pIntValue)
{
    return MV_CC_GetIntValue(m_hDevHandle, strKey, pIntValue);    // 获取 Integer 属性值
    // m_hDevHandle：设备句柄； strKey：属性键值； pIntValue：返回给调用者有关设备属性结构体指针
}
...
// Add
// ch:采用超时机制获取一帧图片
int CMvCamera::GetOneFrameTimeout(unsigned char * pData, unsigned int nDataSize, MV_FRAME_OUT_INFO_EX* pFrameInfo, unsigned int nMsec)
{
...
int CMvCamera::FeatureSave(const char* pFileName)
{
    return MV_CC_FeatureSave(m_hDevHandle, pFileName);
}

// ch:从文件中导入相机属性 | en:Import the camera properties from the file
int CMvCamera::FeatureLoad(const char* pFileName)
{
    return MV_CC_FeatureLoad(m_hDevHandle, pFileName);
}

int CMvCamera::SetCommandValue(const char* strKey)
{
    return MV_CC_SetCommandValue(m_hDevHandle, strKey);
}

int CMvCamera::Display(void * m_widgethWnd)
{
    return MV_CC_Display(m_hDevHandle, m_widgethWnd);
}

// ch:输入录像数据
int CMvCamera::InputOneFrame(MV_CC_INPUT_FRAME_INFO* pstInputFrameInfo)
{
    return MV_CC_InputOneFrame(m_hDevHandle,pstInputFrameInfo);
}

// ch:开始录像
int CMvCamera::StartRecord(MV_CC_RECORD_PARAM* pstRecordParam)
{
    return MV_CC_StartRecord(m_hDevHandle,pstRecordParam);
}

// ch:停止录像
int CMvCamera::StopRecord()
{
    return MV_CC_StopRecord(m_hDevHandle);
}

// ch:插值算法类型设置
int CMvCamera::SetBayerCvtQuality(unsigned int BayerCvtQuality)
{
    return MV_CC_SetBayerCvtQuality(m_hDevHandle,BayerCvtQuality);
}

// ch:注册RGB彩色图像数据回调
int CMvCamera::RegisterImageCallBackForRGB(void(__stdcall* cbOutput)(unsigned char * pData, MV_FRAME_OUT_INFO_EX* pFrameInfo, void* pUser), void* pUser)
{
    return MV_CC_RegisterImageCallBackForRGB(m_hDevHandle, cbOutput, pUser);
}

// ch:注册BGR彩色图像数据回调
int CMvCamera::RegisterImageCallBackForBGR(void(__stdcall* cbOutput)(unsigned char * pData, MV_FRAME_OUT_INFO_EX* pFrameInfo, void* pUser), void* pUser)
{
