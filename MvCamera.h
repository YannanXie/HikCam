...
public:
    ...
    // ch:显示一帧图像 | en:Display one frame image
//    int DisplayOneFrame1(MV_DISPLAY_FRAME_INFO* pDisplayInfo);
    ...
    int GetIntValue2(IN const char* strKey, OUT MVCC_INTVALUE *pIntValue);
    ...
    // ch:采用超时机制获取一帧图片 | en:
    int GetOneFrameTimeout(unsigned char * pData, unsigned int nDataSize, MV_FRAME_OUT_INFO_EX* pFrameInfo, unsigned int nMsec);

    // ch:保存图片为文件 | en:Save the image as a file
    int SaveImageToFile(MV_SAVE_IMAGE_PARAM_EX* pstParam);

    // ch:将相机属性导出到文件中 | en:Export the camera properties to the file
    int FeatureSave(const char* pFileName);

    // ch:从文件中导入相机属性 | en:Import the camera properties from the file
    int FeatureLoad(const char* pFileName);

    int SetCommandValue(const char* strKey);

    int Display(void * m_widgethWnd);

    // ch:输入录像数据
    int InputOneFrame(MV_CC_INPUT_FRAME_INFO* pstInputFrameInfo);

    // ch:开始录像
    int StartRecord(MV_CC_RECORD_PARAM* pstRecordParam);
    int StopRecord();

    // ch:插值算法类型设置
    int SetBayerCvtQuality(unsigned int BayerCvtQuality);

    // ch:注册RGB彩色图像数据回调
    int RegisterImageCallBackForRGB(void(__stdcall* cbOutput)(unsigned char * pData, MV_FRAME_OUT_INFO_EX* pFrameInfo, void* pUser), void* pUser);

    // ch:注册BGR彩色图像数据回调
    int RegisterImageCallBackForBGR(void(__stdcall* cbOutput)(unsigned char * pData, MV_FRAME_OUT_INFO_EX* pFrameInfo, void* pUser), void* pUser);
